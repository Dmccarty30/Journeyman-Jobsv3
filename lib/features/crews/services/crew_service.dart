...
class CrewService {
  ...
  Future<void> _incrementInvitationCount(String userId) async {
    ...
    final lifetimeCounterRef = _firestore
        .collection('counters')
        .doc('invitations')
        .collection('lifetime')
        .doc(userId);
  ...
  Future<T> _retryWithBackoff<T>({
    ...
    throw AppException('Operation failed after multiple retries',
        code: 'retry-failed');
  }
  ...
  Future<String> createCrew({
    ...
    try {
      ...
      if (nameError != null)
        throw CrewException(nameError, code: 'invalid-crew-name');

      final isUnique = await CrewValidation.isCrewNameUnique(name, _firestore);
      if (!isUnique)
        throw CrewException('Crew name already exists',
            code: 'crew-name-exists');
      ...
      if (!await _checkCrewCreationLimit(foremanId)) {
        throw CrewException('Maximum crew limit reached (3 crews per user)',
            code: 'crew-limit-reached');
      }

      if (!_connectivityService.isOnline) {
        // Offline: Store crew locally and mark as dirty
        final crewId =
            'offline_${DateTime.now().millisecondsSinceEpoch}'; // Generate a temporary ID for offline
        final now = DateTime.now();
        ...
        await _offlineDataService.markDataDirty(
            'crew_$crewId', crew.toFirestore());
        await _offlineDataService.markDataDirty(
            'member_${foremanId}_$crewId', member.toFirestore());
        return crewId; // Return the offline crew ID
      }

      String? createdCrewId;
      await _retryWithBackoff(operation: () async {
        final crewId = await _getNextCrewId(name);
        createdCrewId = crewId; // Capture the ID
        final now = DateTime.now();
        ...
        await _firestore.runTransaction((transaction) async {
          transaction.set(
              _firestore.collection('crews').doc(crewId), crew.toFirestore());
          transaction.set(
            _firestore
                .collection('crews')
                .doc(crewId)
      ...
      return createdCrewId!; // Return the created crew ID
    } on CrewException {
      rethrow; // Rethrow custom exceptions directly
    } on FirebaseException catch (e) {
      throw CrewException('Firestore error creating crew: ${e.message}',
          code: e.code);
    } catch (e) {
      throw CrewException(
          'An unexpected error occurred while creating crew: $e',
          code: 'unknown-error');
    }
  }
    ...
    try {
      if (_connectivityService.isOnline) {
        final doc = await _retryWithBackoff(
            operation: () => _firestore.collection('crews').doc(crewId).get());
        if (doc.exists) {
          final crew = Crew.fromFirestore(doc);
          await _offlineDataService
              .storeCrewsOffline([crew]); // Cache the fetched crew
          return crew;
        }
      } else {
        // Attempt to get from offline cache
        final offlineCrews = await _offlineDataService.getOfflineCrews();
        return offlineCrews.firstWhere((crew) => crew.id == crewId,
            orElse: () => throw CrewException('Crew not found in offline cache',
                code: 'crew-not-found-offline'));
      }
      return null;
    } on FirebaseException catch (e) {
      throw CrewException('Firestore error getting crew: ${e.message}',
          code: e.code);
    } catch (e) {
      throw CrewException('An unexpected error occurred while getting crew: $e',
          code: 'unknown-error');
    }
  }
  ...
  Future<void> updateCrew({
    ...
    try {
      if (name != null) {
        ...
        final isUnique =
            await CrewValidation.isCrewNameUnique(name, _firestore);
        if (!isUnique)
          throw CrewException('Crew name already in use',
              code: 'crew-name-exists');
      }
      ...
      if (updates.isNotEmpty) {
        if (!_connectivityService.isOnline) {
          ...
          if (crewIndex != -1) {
            ...
            await _offlineDataService.markDataDirty(
                'crew_$crewId', updatedCrew.toFirestore());
          } else {
      ...
      throw CrewException('Firestore error updating crew: ${e.message}',
          code: e.code);
    } catch (e) {
      throw CrewException(
          'An unexpected error occurred while updating crew: $e',
          code: 'unknown-error');
    }
  }
  ...
  Future<void> deleteCrew(String crewId) async {
    try {
      if (!_connectivityService.isOnline) {
        ...
        if (crewIndex != -1) {
          final updatedCrew =
              existingCrews[crewIndex].copyWith(isActive: false);
          existingCrews[crewIndex] = updatedCrew;
          await _offlineDataService.storeCrewsOffline(existingCrews);
          await _offlineDataService
              .markDataDirty('crew_$crewId', {'isActive': false});
        } else {
          throw CrewException('Crew not found in offline cache',
              code: 'crew-not-found-offline');
        }
      ...
      throw CrewException(
          'An unexpected error occurred while deleting crew: $e',
          code: 'unknown-error');
    }
  }
  ...
  Future<String> inviteMember({
    ...
    try {
      ...
      if (!CrewValidation.isUnderMemberLimit(crew.memberCount)) {
        throw CrewException('Crew has reached maximum member limit (50)',
      ...
      if (message != null) {
        ...
        if (msgError != null)
          throw MessagingException(msgError, code: 'invalid-message-content');
      }

      if (!await _checkOverallInvitationLimit(inviterId)) {
        throw CrewException(
            'Maximum lifetime invitation limit reached (100 invitations)',
            code: 'lifetime-invite-limit-reached');
      }

      if (!await _checkInvitationLimit(inviterId)) {
        throw CrewException('Daily invitation limit reached (max 5 per day)',
            code: 'daily-invite-limit-reached');
      }
      ...
      if (existingMember.exists) {
        throw MemberException('User is already a member of this crew',
            code: 'already-member');
      }

      final existingInvite = await _getInvitation(crewId, inviteeId);
      if (existingInvite != null && existingInvite['status'] == 'pending') {
        throw MemberException('Invitation already pending for this user',
      ...
      throw CrewException('Firestore error inviting member: ${e.message}',
          code: e.code);
    } catch (e) {
      throw CrewException(
          'An unexpected error occurred while inviting member: $e',
          code: 'unknown-error');
    }
  }
  ...
  Future<void> removeMember({
    ...
    try {
      ...
      if (!await hasPermission(
          crewId: crewId,
          userId: inviterId,
          permission: Permission.removeMember)) {
        throw CrewException('Insufficient permissions to remove members',
            code: 'permission-denied');
      }
      ...
      throw CrewException('Firestore error removing member: ${e.message}',
          code: e.code);
    } catch (e) {
      throw CrewException(
  ...
  Future<void> updateMemberRole({
    ...
    try {
      if (!await hasPermission(
        ...
        throw CrewException('Insufficient permissions to update roles',
            code: 'permission-denied');
      }
      ...
      if (!_connectivityService.isOnline) {
        ...
        if (memberIndex != -1) {
          ...
          throw MemberException('Member not found in offline cache',
              code: 'member-not-found-offline');
        }
        return;
      }
      ...
      throw CrewException('Firestore error updating member role: ${e.message}',
          code: e.code);
    } catch (e) {
      throw CrewException(
          'An unexpected error occurred while updating member role: $e',
          code: 'unknown-error');
    }
  }
  ...
  Future<void> acceptInvitation({
    ...
    try {
      if (!_connectivityService.isOnline) {
        ...
        final roleStr = (await _offlineDataService
                .getOfflineUserPreferences())?['invitations']?['role'] ??
            'member'; // Placeholder for role
        final member = CrewMember(
          uid: userId,
          crewId: crewId,
          role: roleStr,
      ...
      await _firestore.runTransaction<void>((transaction) async {
        ...
        if (!inviteDoc.exists)
          throw MemberException('Invitation not found',
        ...
        if (status != 'pending')
          throw MemberException('Invitation is no longer pending',
              code: 'invitation-not-pending');
        if (userId != inviteeId)
          throw MemberException('Unauthorized to accept this invitation',
              code: 'unauthorized-accept');
        if (expiresAt != null && DateTime.now().isAfter(expiresAt)) {
          transaction.update(inviteRef, {
            'status': 'expired',
            'updatedAt': FieldValue.serverTimestamp(),
          });
          throw MemberException('Invitation has expired',
      ...
      throw CrewException('Firestore error accepting invitation: ${e.message}',
          code: e.code);
    } catch (e) {
      throw CrewException(
          'An unexpected error occurred while accepting invitation: $e',
          code: 'unknown-error');
    }
  }
  ...
  Future<void> rejectInvitation({
    ...
    try {
      ...
      throw MemberException(
          'Firestore error rejecting invitation: ${e.message}',
          code: e.code);
    } catch (e) {
      throw MemberException(
          'An unexpected error occurred while rejecting invitation: $e',
          code: 'unknown-error');
    }
  }
  ...
  Future<void> cancelInvitation({
    ...
    try {
      ...
      if (data['inviterId'] != inviterId)
        throw MemberException('Only inviter can cancel this invitation',
            code: 'unauthorized-cancel');

      if (data['status'] == 'accepted')
        throw MemberException('Cannot cancel an accepted invitation',
      ...
      await userInviteRef
          .delete()
          .catchError((_) {}); // Catch error for delete if doc doesn't exist
    } on AppException {
      rethrow;
    } on FirebaseException catch (e) {
      throw MemberException(
          'Firestore error cancelling invitation: ${e.message}',
          code: e.code);
    } catch (e) {
      throw MemberException(
          'An unexpected error occurred while cancelling invitation: $e',
          code: 'unknown-error');
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getPendingInvitations(
    ...
    try {
      ...
      throw MemberException(
          'Firestore error getting pending invitations: ${e.message}',
          code: e.code);
    } catch (e) {
      throw MemberException(
          'An unexpected error occurred while getting pending invitations: $e',
          code: 'unknown-error');
    }
  }
  ...
  Future<void> cleanupExpiredInvitations(String crewId) async {
    try {
      ...
      throw MemberException(
          'Firestore error cleaning up expired invitations: ${e.message}',
          code: e.code);
    } catch (e) {
      throw MemberException(
          'An unexpected error occurred while cleaning up expired invitations: $e',
          code: 'unknown-error');
    }
  }

  // Invitation helpers
  Future<Map<String, dynamic>?> _getInvitation(
      String crewId, String inviteeId) async {
    final snapshot = await _firestore
        .collection('crews')
        .doc(crewId)
        .collection('invitations')
    ...
    await _firestore.runTransaction<void>((transaction) async {
      ...
      if (matchScore != null) {
        ...
        if (newScores.length > 50)
          newScores = newScores.sublist(newScores.length - 50);
      }
  ...
  Future<double> _calculateAverageResponseTime(String crewId) async {
    ...
    for (final doc in applicationsSnapshot.docs) {
      final data = doc.data();
      final suggestedAt =
          (data['suggestedAt'] as Timestamp?)?.toDate() ?? DateTime.now();
      final appliedAt =
          (data['appliedAt'] as Timestamp?)?.toDate() ?? DateTime.now();
      final responseMinutes =
          appliedAt.difference(suggestedAt).inMinutes.toDouble();
  ...
  Future<bool> validateCrewStats(String crewId) async {
    try {
      ...
      if (currentStats.applicationRate > totalApps / (shared > 0 ? shared : 1))
        return false;
      if (currentStats.averageMatchScore < 0 ||
          currentStats.averageMatchScore > 100) return false;
      if (currentStats.responseTime < 0) return false;
      if (currentStats.successRate < 0 || currentStats.successRate > 1)
        return false;
  ...
  Stream<QuerySnapshot> getUserCrewMembersStream(String crewId, String userId) {
    return _firestore
            ...
            .snapshots()
            .asBroadcastStream()
        as Stream<QuerySnapshot>; // Fix return type if needed
  }

  Future<List<Crew>> getUserCrews(String userId) async {
    try {
      if (_connectivityService.isOnline) {
        final memberSnapshot = await _retryWithBackoff(
        ...
        final crews =
            (await Future.wait(crewFutures)).where((c) => c.isActive).toList();
        await _offlineDataService
            .storeCrewsOffline(crews); // Cache the fetched crews
        return crews;
      } else {
        // Attempt to get from offline cache
        final offlineCrews = await _offlineDataService.getOfflineCrews();
        ...
        return offlineCrews
            .where(
                (crew) => userJoinedCrewIds.contains(crew.id) && crew.isActive)
            .toList();
      }
    } on FirebaseException catch (e) {
      throw CrewException('Firestore error getting user crews: ${e.message}',
          code: e.code);
    } catch (e) {
      throw CrewException(
          'An unexpected error occurred while getting user crews: $e',
          code: 'unknown-error');
    }
  }
  ...
  Future<List<CrewMember>> getCrewMembers(String crewId) async {
    try {
      if (_connectivityService.isOnline) {
        ...
        final members =
            snapshot.docs.map((doc) => CrewMember.fromFirestore(doc)).toList();
        await _offlineDataService
            .storeCrewMembersOffline(members); // Cache the fetched members
        return members;
      } else {
        // Attempt to get from offline cache
        final offlineMembers =
        ...
        return offlineMembers
            .where((member) => member.crewId == crewId)
            .toList();
      }
    } on FirebaseException catch (e) {
      throw MemberException(
          'Firestore error getting crew members: ${e.message}',
          code: e.code);
    } catch (e) {
      throw MemberException(
          'An unexpected error occurred while getting crew members: $e',
          code: 'unknown-error');
    }
  }
    ...
    try {
      if (_connectivityService.isOnline) {
        ...
        if (doc.exists) {
          ...
          await _offlineDataService
              .storeCrewMembersOffline([member]); // Cache the fetched member
          return member;
        }
      } else {
        // Attempt to get from offline cache
        ...
        return offlineMembers.firstWhere(
            ...
            orElse: () => throw MemberException(
                'Crew member not found in offline cache',
                code: 'member-not-found-offline'));
      }
      return null;
    } on FirebaseException catch (e) {
      throw MemberException('Firestore error getting crew member: ${e.message}',
          code: e.code);
    } catch (e) {
      throw MemberException(
          'An unexpected error occurred while getting crew member: $e',
          code: 'unknown-error');
    }
  }
